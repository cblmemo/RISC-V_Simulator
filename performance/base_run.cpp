...
/*
 * This version implemented two-level predictor.
 * */
class CPU {
private:
...
#pragma endregion
    ...
    private:
        ...
        SaturatingCounter sc[1 << n];
        uint history = 0;
    
    public:
        void update(bool taken) {
            sc[history].update(taken);
            history <<= 1;
            if (taken)history |= 1;
            history &= (0xffffffff >> (32 - n));
        }
        
        [[nodiscard]] bool predict() const {
            return sc[history].predict();
        }
    ...
    void InstructionFetch() {
        ...
        if (isBranchInst(f2dReg.instruction)) {
            if (branchPredict[hashPC(pc)].predict()) {
                ...
                if (tarAddr != (uint) -1)f2dReg.tarAddr = pc = tarAddr;
...
//        std::cout << (((uint) reg[10]) & 0x000000ff) << std::endl;
    ...
    void printSuccessRate() const {
        std::cout << (double) (predictSuccess * 100) / (double) (predictSuccess + predictFailure)
                  << "\n" << predictSuccess << "\n" << predictSuccess + predictFailure << std::endl;
    }
};
